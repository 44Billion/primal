...
export class BunkerSigner implements NostrSigner {
        ...
        { kinds: [NostrConnect], '#p': [clientPubkey] },
        {
          onevent: async (event: NostrEvent) => {
            try {
              ...
              if (secret === parsedURI.params.secret) {
                clearTimeout(timer)
                sub.close()
  ...
  async sendRequest(method: string, params: string[]): Promise<string> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // the request event
        const verifiedEvent: NostrVerifiedEvent = finalizeEvent(
          {
            kind: NostrConnect,
   ...
   */
  async signEvent(event: NostrEventTemplate): Promise<NostrVerifiedEvent> {
...
export async function createAccount(
  ...
  domain: string,
  email?: string,
  localSecretKey: Uint8Array = generatePrivateKey(),
): Promise<BunkerSigner> {
